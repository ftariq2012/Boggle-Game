...
public class BoggleGame {
    ...
    private String randomizeLetters(int size){
        ...
        if(size==4){
            for(String st: dice_small_grid){
                int index = (int) Math.random() * 6;
                randomLetters+= st.charAt(index);
            }
        }
        else if(size==5){
            for(String st: dice_big_grid){
                int index = (int) Math.random() * 6;
                randomLetters+= st.charAt(index);
            }
        }
    ...
    private void findAllWords(Map<String,ArrayList<Position>> allWords, Dictionary boggleDict, BoggleGrid boggleGrid) {
        boolean flag = true;
        String grid = boggleGrid.toString();
        grid = grid.replaceAll(" ","");
        grid = grid.replaceAll("\n","");
        for(String word: boggleDict.dict()){
            String dicWord = "";
            flag = true;
            ...
            for(char c: word.toCharArray()){
                if(grid.contains(c+"")){
                    ...
                    i = index/boggleGrid.numRows();
                    j = index%boggleGrid.numCols();
                    Position p = new Position(i,j);
                    if(positions.contains(p)==false){
                        positions.add(p);
                        dicWord += c;
                    }
                }
            ...
            if(flag){
                allWords.put(dicWord, positions);
            }
        }
